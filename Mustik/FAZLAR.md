...
4. Uçtan uca bir sipariş isteği tetikleyip Jaeger UI'da `trace_id` ile arama yap *(Yeni bir sipariş tetiklendi, Jaeger API'sinden trace_id alınıp `localhost:16686/trace/{id}` — UI'daki "Lookup by Trace ID" kutusuyla aynı işlev — ile arandı; trace başarıyla yüklendi: 310.98ms, 3 servis, 7 span)* ✅
5. 4 servisin span'lerinin tek bir trace timeline'ında göründüğünü doğrula *(Değerlendirildi — Faz 10 madde 7'de tespit edilen mimari gerçeği burada da geçerli: "sipariş" tek bir HTTP isteği olmadığından tek bir trace 4 servisin tamamını kapsamıyor. En geniş zincir olan `POST /payment` trace'i 3 servisi (payment+cart+inventory) tek timeline'da gösteriyor; search-service yalnızca ayrı bir kök istek olan "ürün ekle" trace'inde (cart+search) görünüyor. Bu bir kopukluk değil, mimarinin doğal sonucu)* ✅
6. Her span'in süresini (duration) inceleyip hangi servisin en çok zaman aldığını gözlemle *(İncelendi — `POST /payment` trace'inde (310.98ms toplam) `payment-service`'in `bank-charge` span'i 300.17ms ile toplamın ~%96.5'ini oluşturuyor; diğer tüm span'ler (cart-service GET 1.84ms, inventory-service reserve 2.38ms, reserve-stock ~37µs) milisaniyenin altında. Bank API simülasyonu (`BankApiClient`) net bir şekilde darboğaz olarak görünüyor)* ✅
7. Kasıtlı olarak Payment Service'e sabit bir gecikme (örn. `time.sleep(0.5)`) ekleyip Jaeger'da bunun görünürlüğünü test et, sonra kaldır *(`BankApiClient.SIMULATED_LATENCY_MILLIS` geçici olarak 300→3500ms yapıldı, rebuild edilip test edildi: istek gerçekten ~3.86s sürdü, Jaeger'da `bank-charge` span'i 3500.26ms olarak net görüldü (root span 3829ms). Ardından 300ms'e geri alınıp rebuild edildi, normale (~0.9s) döndüğü doğrulandı)* ✅
...
10. Inventory Service'te `stock_level` gauge metriği ekle *(`StockRepository`'ye `findAll()` eklendi, yeni `StockMetrics` bileşeni `@PostConstruct`'ta her ürün için `stock_level{product_id=...}` gauge'i (`stockCount - reservedCount`, canlı okuma) kaydediyor. `mvn test` geçti; canlı doğrulama: `reserve` sonrası 42.0→41.0, `release` sonrası 41.0→42.0)* ✅
11. Prometheus scrape interval'ını (örn. 5s-15s) değerlendir ve ayarla *(Zaten `10s` — önerilen aralığın ortası, dev ortamı için makul; değişiklik gerekmedi)* ✅
12. Prometheus config dosyasını `infra/prometheus.yml` altında versiyonla *(Zaten bu yolda oluşturulmuş ve git'te takip ediliyor)* ✅
13. README'ye "Metrics — Prometheus kurulumu" bölümü ekle *(README'ye "Gün 12 Durumu" bölümü eklendi: veri akışı diyagramı, RED metrikleri, scrape config, custom metrikler, PromQL doğrulaması)* ✅
14. Git commit
